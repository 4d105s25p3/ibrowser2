...
var resetMenu = function(level = 0, removeResults = true) {
	...
	currentSampleSelection = new Array();
	...
	if (this.readyState == 4 && this.status == 200) {
		...
		confirm.on('click', function(d, i) {
			...
			if (selected.length != 0) { // at least one sample is selected
				...
				newSampleSelection.sort(function(sample1, sample2) { return sample1.localeCompare(sample2); });
				currentSampleSelection = Array.from(currentSamples);
				currentSampleSelection.sort(function(sample1, sample2) { return sample1.localeCompare(sample2); });
					...
					.on("click", function(d, i){ 
						...
						if (!document.getElementById(url)) {
							drawTree.text("Draw Tree")
								.on("click", function(d, i){
									var $TreeCanvas = $("<div>", {"class": "tree-canvas " + url}).appendTo("#tree-panel");							
									$TreeCanvas.dialog({
								        'autoOpen': false,
								        'height': Math.round($(window).height()*0.95) - 350,
								        'width': Math.round($(window).height()*0.95) - 350,
								        "left": "350px",
								        "top": "350px",
								    });

									var $rTable = $("<table>", {"class": "reportTable " + url});
									$rTable.appendTo($TreeCanvas);
					                ...
					                var trs     = new Array(pairs.length + 2);
								    for ( ppos = 0; ppos < pairs.length; ++ppos ) {
								        var pval  = pairs[ppos];
								        trs[ppos] = $('<tr>', {'class': 'reportLine ' + pval[0]})
								                    .append( $('<td>', { 'class': 'reportCell reportHeader' }).html( pval[0] ) )
								                    .append( $('<td>', { 'class': 'reportCell reportVal'    }).html( pval[1] ) );
								    }
								    trs[5] = $('<tr>', {'class': 'reportLine tree'})
								    		 .append( $('<td>', { 'class': 'reportCell reportHeader' }).html('Tree'))
								    		 .append( $('<td>', { 'class': 'reportCell reportVal' })
								    		 	.append( $('<div>', { 'id': url })));
									...
									drawTree.property("disabled", false)
										.text("Show Tree")
										.on("click", function(d, i){
											$TreeCanvas.dialog("open");
										});
								});
						} else {
							drawTree.text("Show Tree")
								.on("click", function(d, i){
									var $TreeCanvas = $("div[class='tree-canvas " + url + " ui-dialog-content ui-widget-content']")
									$TreeCanvas.dialog("open");
								});
						}
					})
